...
public class PackageInstallerActivity extends BaseActivity implements InstallerDialogHelper.OnClickButtonsListener {
    ...
    private void displayInstallationPrompt(int actionRes, boolean splitOnly) {
        ...
        if (!splitOnly) {
            ...
            showInstallConfirmationDialogAsync(actionRes, R.string.cancel, this, mModel.getTrackerCount(),
                    new ArrayList<>(mPendingDependencyIssues), mCurrentItem);
        } else triggerInstall();
    }

    @UiThread
    private void showInstallConfirmationDialogAsync(@StringRes int installButtonRes,
                                                    @StringRes int cancelButtonRes,
                                                    @NonNull InstallerDialogHelper.OnClickButtonsListener listener,
                                                    int trackers,
                                                    @NonNull List<InstallDependencyChecker.Issue> dependencyIssues,
                                                    @Nullable ApkQueueItem checkedItem) {
        Context appContext = getApplicationContext();
        ThreadUtils.postOnBackgroundThread(() -> {
            InstallerPrivilegeCascade.Plan computedPrivilegePlan;
            try {
                computedPrivilegePlan = InstallerPrivilegeCascade.getPreviewPlan(appContext);
            } catch (Exception e) {
                Log.w(TAG, "Could not build installer privilege preview plan.", e);
                computedPrivilegePlan = InstallerPrivilegeCascade.buildPlan(false, false, false, false, false, false);
            }
            InstallerPrivilegeCascade.Plan privilegePlan = computedPrivilegePlan;
            ThreadUtils.postOnMainThread(() -> {
                if (isFinishing() || checkedItem != mCurrentItem) {
                    return;
                }
                CharSequence callout = composeInstallCallout(trackers, dependencyIssues, privilegePlan);
                mDialogHelper.showInstallConfirmationDialog(installButtonRes, cancelButtonRes, listener,
        ...
        if (ACTION_PACKAGE_INSTALLED.equals(intent.getAction())) {
            ...
            try {
                if (mPackageName == null || confirmIntent == null) {
                    throw new Exception("Empty confirmation intent (" + decision + ").");
                }
    ...
    private void triggerBatchInstall() {
        try {
            ...
            if (!mBatchDowngradeWarningShown
                ...
                showInstallConfirmationDialogAsync(R.string.downgrade, R.string.skip,
                        ...
                        }, mModel.getTrackerCount(), new ArrayList<>(mPendingDependencyIssues), mCurrentItem);
                return;
            }
            mDeveloperVerificationWarningShown = true;
            triggerInstall();
