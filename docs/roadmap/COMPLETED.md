...
- [x] **P1 AppManagerNG shipped docs and privacy rebaseline** — packaged
  manual identity, official distribution/source links, translation guidance,
  donation copy, and contact surfaces now point at AppManagerNG/SysAdminDoc
  instead of upstream App Manager. `BUILDING.rst`, `PRIVACY_POLICY.rst`,
  `README.md`, `CONTRIBUTING.md`, `PROJECT_CONTEXT.md`, and default English UI
  copy now describe the fork's current package identity, release channel,
  `floss`/`full` split, optional network features, VirusTotal/Pithus/debloat
  behavior, and support links. Verification:
  `:docs:buildDocs :app:processFlossDebugResources :app:compileFlossDebugJavaWithJavac`.
  Full JVM suite remains blocked by an Android Studio JBR/Robolectric VM crash
  observed during this pass; focused touched-area test result XML recorded 23
