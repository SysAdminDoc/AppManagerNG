...
**Last updated:** 2026-05-06 (iter-19: community pain-point sweep + Android 17 Beta 4 platform deltas + Shizuku-era competitor harvest — Hidden-API compat harness, GCM-cipher reuse on large OBB, Shizuku-perm-revoke-on-clear warning, OEM debloat-blocker bypass, per-app rollback, FireOS SYSTEM USER privilege, Blocker IFW editor parity, Amarok-Hider `pm hide` toggle, Language-Selector per-app locale, KernelSU ADB-Root privilege enum, F-Droid 2.0 index v2 + ROM JSON format, ACCESS_LOCAL_NETWORK + static-final reflection ban + ML-DSA cert OIDs).
...
| Item | Description | Theme | Tier |
|------|-------------|-------|------|
| **Hidden-API Compatibility Harness** | Upstream maintainer documented that *"a migration to a new version of Android roughly takes 80 hours alone as it is necessary to revise entire hidden API library"* and Android 16's removal of `clearApplicationUserData` silently broke restore for every NG user on the QPR2 patch ([S137]). NG inherits the same surface with no automated coverage. Add an instrumented test matrix that loads `app/src/main/assets/api/api-versions-*.json` (already bundled), reflectively probes every hidden-API call-site at runtime against the active SDK, and emits a diff report so we catch removals/renames before users do. Effort 4/5; the single highest-leverage eng-debt item we have. | Eng-Debt | **Now** |
| **GCM Cipher Reuse on Large OBB Backup** | Upstream issue #1958 traced a backup-corruption pattern on >2 GB OBB archives to `Cipher` instance reuse across the encrypt-stream chunk boundary; the GCM IV got reset mid-stream and the resulting tag failed verification on restore ([S138]). NG's `BackupCryptoUtils` shares the same lineage. Audit `EncryptedOutputStream` / `EncryptedInputStream` for IV-per-chunk discipline and add a regression test that round-trips a synthetic 4 GB blob. Effort 3/5; concrete data-loss bug. | T6 | **Now** |
| **Shizuku Permission Auto-Revoke Warning** | Canta #359 documented a security-relevant Shizuku quirk: when an app's data is cleared (manually or by NG's "Clear data" action) Shizuku silently revokes its permission grant, but the app keeps showing as "Shizuku-authorized" until the next restart ([S139]). NG should detect data-clear actions on Shizuku-authorized apps, warn pre-action, and re-probe `Shizuku.checkSelfPermission()` post-action with a re-auth deep-link if revoked. Effort 2/5; closes a stale-trust window. | T5 | **Now** |
...
| **Freeze / Operation Audit Log** | Hail #386, #388, #392 all asked for a chronological log of who-froze-what-when and a recovery path after accidental unfreezes that broke system components ([S144], [S145]). NG iter-18 has the "Operation Activity Log" stub but no UI; ship a Settings → Activity Log screen sourcing from the `RoutineOpsHistoryDao` already present, with per-row "Revert this action" surfaced from the rollback work above. Effort 3/5. | T8 | **Now** |
...
| **Android 17 16 KB Page-Size Compatibility** | Pixel 9a Android 17 testers report NG's main menu showing zero apps because some shared libs aren't 16 KB-aligned ([S148]). Iter-18 [S126] added a *device-side* page-size indicator; iter-19 needs the *NG-side* fix: rebuild every native dependency (libsu, libsuperuser, BouncyCastle native) with `-Wl,-z,max-page-size=16384`, audit every `android:extractNativeLibs="false"` APK we ingest, and add a CI gate that fails the release if any `.so` is mis-aligned. Effort 3/5; mainstreams in Pixel 10 lineup within the year. | Eng-Debt | **Now** |
| **Google Play Contacts/Location-Button Policy** | Google Play Console enforcement begins ~2026-05-15 for the Contacts and Location-button policy: any in-app button that reveals personal contact info or precise location must declare a runtime-permission rationale dialog before exposing the data ([S171]). NG's manifest declares `READ_CONTACTS` (for the export-contacts utility) and `ACCESS_FINE_LOCATION` (none today, audit). Audit every UI surface that surfaces contact info or location and confirm rationale dialogs fire pre-disclosure. Effort 1/5; pure compliance pass, must land before May 15. | Eng-Debt | **Now** |
| **KernelSU ADB-Root Privilege Mode Enum** | KernelSU v3.2.3/3.2.4 added an "ADB Root" toggle that grants `adb shell` runtime root without `ro.debuggable=1` ([S166]). NG's privilege detector currently classifies `adb shell` as ADB-mode regardless. Add a new `PrivilegeMode.KERNELSU_ADB_ROOT` enum that is detected when KernelSU is present *and* `adb shell id` returns uid 0, with explicit user confirmation before NG trusts it (anyone can plug in via USB). Effort 2/5. | T9 | **Now** |
