...
- Source-backed 2026-06-06 audit closures now include backup metadata
  ...
  hardening, backup metadata archive-type hardening, backup metadata timestamp
  hardening, backup metadata user-id hardening, backup crypto mode parsing
  hardening, backup metadata parse-error wrapping, backup checksum row generation
  ...
  scalar parser hardening, rule numeric negativity hardening, app-op rule numeric
  hardening, freeze rule method hardening, component rule status-token hardening,
  rule import boolean token hardening, ADB encrypted key-blob length hardening,
  ADB backup
