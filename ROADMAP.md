...
- Source-backed 2026-06-06 audit closures now include automation component-name
  validation, package-name empty-segment validation, Hail profile empty-import
  rejection, external profile package-name validation, logcat search criteria
  hardening, logcat numeric field parser hardening, Titanium Backup metadata
  parser hardening, default-app role holder parser hardening, selected-user
  preference parser hardening, rule import TSV required-field hardening, URI
  grant flattened parser hardening, Activity Interceptor pasted header
  hardening, intent empty primitive-array serialization, intent `CharSequence`
  ...
  named-actor conversion hardening, SysConfig associated-package label display,
  File properties shared-UID label display, intent flattened string empty/tab
  extra parsing, Finder matched result evidence display, ADB backup header hex
