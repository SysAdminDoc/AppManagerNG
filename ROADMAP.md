...
- [ ] P2 — 2026-06-09 deferred-audit reliability batch
  Why: The deep audit deferred verified-real, low-individual-cost reliability bugs that were lost when the old ROADMAP section was replaced; re-itemizing them prevents silent drop: SAF pending-write fields lost on process death + profile-export main-thread IO; OneClickOps onPause busy-clear without scan-cancel; retention pruners leaving stale Room rows with no broadcast; commit() skipping the previous-backup freeze flag.
