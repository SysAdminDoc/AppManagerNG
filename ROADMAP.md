...
**Last updated:** 2026-05-03 (factory iter-18: Shizuku v13.6.0 trusted-WLAN auto-start awareness banner, Magisk v30.7 `--drop-cap` opt-in semantics surface, KernelSU v3.2 sulog/seccomp parity, Android 16 `SDK_INT_FULL` minor-version plumbing, JobScheduler quota stop-reason surfacing, Termux SE_PROCESS_CONTEXT/SE_FILE_CONTEXT info parity, APKEditor smali comment-level "basic" + `-remove-annotation` flags, Apktool 3.0.x "Remastered" upgrade, JADX 1.5.5 `.apks` ingestion + UI zoom, Hail v1.10.0 Auto-Freeze QS Tile + Digital-Assistant launch + KernelSU App Profile, Inure build107 batch-installer + AppOps IGNORE flag + native-lib sizes, Material Files v1.7.4 checksum-properties tab + WebDAV self-signed cert trust, SD Maid SE auto-fix battery optimization via root/ADB + warn-before-volume-scan, UAD-NG v1.2.0 cross-user package state verification + auto-fetch debloat definitions + selectable descriptions, Neo Backup 8.3.17 backup sharing + tag suggestion + KeepAndroidOpen banner, glance widgets parity audit, Compose 1.6 stability, S121–S132; iter-17: TalkBack/screen reader audit, reduced motion support, dyslexia-friendly font, Material 3 Expressive migration, Compose-stable VM states, launcher shortcuts for schedules, backup sharing button, Material 3 result notifications, auto-fix battery optimization via root/ADB, cross-user package state detection, auto-update debloat definitions, schedule result detail notifications, searchable schedule tags, KeepAndroidOpen banner, large APK pre-install warning, APKM/XAPK format support, Android TV launcher manifest, publish signing-cert fingerprint URL, S112–S120; iter-16: Material 3 Adaptive layouts for tablets, fast list rendering for 10k+ apps, landscape density overrides, attention badges, undo/SnackBar, discreet icon mode, in-app language selector, custom actions, Dark Mode + Material You integration, smooth transitions/animations (T21 UI/Design Polish — new theme), cross-referenced premium OSS exemplars (Droid-ify, Island, Compose Multiplatform, android-browser-helper, Now in Android, Auxio, Mullvad VPN, ProtonMail), S104–S111; iter-15: Frida-based runtime hook tracing UC, JADX decompiler GUI integration + split APK installer in T12, Performance & Profiling (T20) new theme with Perfetto/LeakCanary/simpleperf/memory inspector, S93–S103; iter-14: MIME type recognition for file manager, structured APK editing architecture guidance, non-root APK module injection framework UC exploration, S89–S92; iter-13: battery optimization bypass, Android 16 capability dropping UI, VPN plugin flags control, launcher backup restoration, S84–S88; iter-12: ZygiskNext detection, installer APK SHA256 toast, SMB/WebDAV backup destination, URI/Intent automation API, package visibility analysis, install source attribution detail, Android 15 app archiving UC, Apktool 3.x migration eng debt, S77–S83; iter-11: APatch as 4th root path, Privilege Health-Check Screen, APK signing scheme display, permission provider attribution, app state snapshot/diff, per-lib 16KB alignment indicator, APK URL metadata preview, native lib extraction, bloatware safety rating in App Detail promoted to T7, predictive back eng debt, S72–S76; iter-10: Dhizuku DeviceOwner privilege path, Force Stop via Shizuku, AppOps quick-toggle in list, Notification.ProgressStyle for backup, Force DEX compile optimization, S71; iter-9: ProfilingManager OOM triggers, graduated tracker blocking modes, per-app tracker stats, permission change monitor, PQC signing pipeline note, DDG Tracker Radar license flag, AlarmManager OnAlarmListener note, VirusTotal poll TODO, S69–S70; iter-8: multi-tag per app, relevance/description search, permission policy flags, native lib sizes, batch APK installer, Android 17 Keystore key cap + Android 18 URI grant preemptive fixes, T19 Package-Aware Storage Analysis, S65–S68; iter-7: Android 17 MessageQueue audit, Adaptive Layout audit, tablet density overrides; iter-6: sort-by-dangerous-perms, elegantTextHeight audit, Obtainium config; baseline post-v0.3.0 fifth full research cycle 2026-05-01). iter-15: Frida-based runtime hook tracing UC, JADX decompiler GUI integration + split APK installer in T12, Performance & Profiling (T20) new theme with Perfetto/LeakCanary/simpleperf/memory inspector, S93–S103; iter-14: MIME type recognition for file manager, structured APK editing architecture guidance, non-root APK module injection framework UC exploration, S89–S92; iter-13: battery optimization bypass, Android 16 capability dropping UI, VPN plugin flags control, launcher backup restoration, S84–S88; iter-12: ZygiskNext detection, installer APK SHA256 toast, SMB/WebDAV backup destination, URI/Intent automation API, package visibility analysis, install source attribution detail, Android 15 app archiving UC, Apktool 3.x migration eng debt, S77–S83; iter-11: APatch as 4th root path, Privilege Health-Check Screen, APK signing scheme display, permission provider attribution, app state snapshot/diff, per-lib 16KB alignment indicator, APK URL metadata preview, native lib extraction, bloatware safety rating in App Detail promoted to T7, predictive back eng debt, S72–S76; iter-10: Dhizuku DeviceOwner privilege path, Force Stop via Shizuku, AppOps quick-toggle in list, Notification.ProgressStyle for backup, Force DEX compile optimization, S71; iter-9: ProfilingManager OOM triggers, graduated tracker blocking modes, per-app tracker stats, permission change monitor, PQC signing pipeline note, DDG Tracker Radar license flag, AlarmManager OnAlarmListener note, VirusTotal poll TODO, S69–S70; iter-8: multi-tag per app, relevance/description search, permission policy flags, native lib sizes, batch APK installer, Android 17 Keystore key cap + Android 18 URI grant preemptive fixes, T19 Package-Aware Storage Analysis, S65–S68; iter-7: Android 17 MessageQueue audit, Adaptive Layout audit, tablet density overrides; iter-6: sort-by-dangerous-perms, elegantTextHeight audit, Obtainium config; baseline post-v0.3.0 fifth full research cycle 2026-05-01).  
...
| **Inure-Style Native-Lib Size Surface** | Inure build107 displays per-`.so` sizes in App Info → Native libs ([S131]). NG already lists native libs but only with name + ABI; adding "Size: 3.2 MB (extracted) / 1.1 MB (compressed)" alongside the alignment indicator gives users a real handle on storage attribution. Effort 1/5. | T19 | **Now** |
...
| **SD-Maid-Style Auto-Fix Battery Optimization** | SD Maid SE v1.7.2-rc0 auto-fixes battery optimization for the scheduler via root or ADB ([S133]). NG just shipped the user-visible Battery Optimization preference (manual flow); when the user is running with root or ADB, NG should silently call `cmd deviceidle whitelist +<package>` instead of opening the system dialog. Effort 2/5; closes the loop on the manual-only fallback. | T20 | **Now** |
...
| **UAD-Style Selectable & Copyable Descriptions** | UAD-NG v1.2.0 made every package description in its UI selectable and copyable ([S134]). NG's App Info "Description" panel and AppOps descriptions are non-selectable; wrapping the relevant `TextView` rows with `android:textIsSelectable="true"` and providing a long-press "Copy" affordance is a 1-line-per-row change. Effort 1/5; trivial accessibility/UX win. | T10 | **Now** |
| **UAD-Style Copy-Error Button on Failure Toasts** | UAD-NG v1.2.0 added a one-tap "Copy error" button on every error toast ([S134]). NG surfaces errors via SnackBar/Toast with no easy copy path; standardizing on a `Snackbar.setAction("Copy", ...)` with `ClipboardManager.setPrimaryClip()` for every error path drastically improves bug-report quality. Effort 2/5. | T4 | **Now** |
...
| **Neo-Backup-Style KeepAndroidOpen Banner** | Neo Backup 8.3.17 surfaces a "Keep Android open during long backups" banner when a backup operation begins on Android 14+ to combat aggressive Doze/standby kills ([S135]). NG's backup operation should display the same banner under Android 14+ given the new JobScheduler quota tightening. Effort 1/5. | T6 | **Now** |
