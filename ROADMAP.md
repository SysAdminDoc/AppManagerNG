...
- Source-backed 2026-06-06 audit closures now include app-op rule numeric
  hardening, freeze rule method hardening, component rule status-token
  hardening, rule import boolean token hardening, ADB encrypted key-blob length
  hardening, ADB backup compression flag hardening, ADB backup numeric header
  hardening, manifest intent-filter name hardening, manifest intent-filter
  priority hardening, dynamic shortcut candidate validation, app action
  shortcut target validation, profile URI package override validation,
