...
| **Android TV / Google TV Launcher Support** | Add `<category android:name="android.intent.category.LEANBACK_LAUNCHER" />` to Manifest so NG appears on Android TV / Google TV home screens; useful for power users debloating TV-box ROMs (e.g. Mi Box, Shield, Onn., Chromecast with Google TV). Reference: SD Maid SE v1.7.x ([S112]) "General: Show app on Android TV / Google TV launchers". Trivial change; significant addressable-user expansion. | Trivial |
...
| **APatch Detection in Capability Wizard** | Detect APatch (kernel-based root via KernelPatch; ARM64, kernel 3.18–6.12) as a fifth root manager alongside Magisk, KernelSU, Dhizuku, and Shizuku. Check `/data/adb/ap/` marker directory or presence of `apd` daemon; surface APatch status, SuperKey availability, and module count in privilege wizard. APatch uses a GPL-3.0 kernel module injection model ([S76]); SD Maid SE v1.5.3 added APatch detection as a parallel precedent ([S42]). | Low |
...
| **ZygiskNext Detection** | Detect ZygiskNext v1.3.x (Zygisk module loader layer on top of KernelSU/APatch) by presence of its marker at `/data/adb/modules/zygisksu/`; surface alongside root manager in the Privilege Health-Check Screen. Surface module error count if ZygiskNext's v1.3.4+ error-check mechanism reports issues. Relevant for users who layer APatch or KernelSU with ZygiskNext for Magisk-module compatibility. ZygiskNext v1.3.4 ([S77]). | Low |
...
| **Battery Optimization Bypass Toggle** | In Privilege Health-Check Screen, add one-tap toggle to request battery-optimization bypass (Dhizuku v2.11.2 uses `requestIgnoreBatteryOptimizations()`); pairs with the existing root/ADB auto-disable toggle. Signals to the OS that NG's ongoing privilege checks need foreground service exemption. Works with or without root. Dhizuku v2.11.2 model ([S84]). | Low |
