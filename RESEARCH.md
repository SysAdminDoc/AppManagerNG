...
[Verified] AppManagerNG v0.6.18 is an offline-first Android package administration suite for privacy-conscious power users. Its strongest current shape is the combination of package inspection, privileged actions, installer preflight, backup conversion, local diagnostics, an API 21 floor, a FLOSS flavor without optional remote-network features, and unusually strict release evidence. The current code is much stronger than the published v0.6.12 release, but its next gains should come from closing narrow trust boundaries rather than adding another broad tool surface. The highest-value direction is to make every privileged or untrusted-input operation explicit about blast radius, lifecycle, authoritative state, and provenance.

Top opportunities, in priority order:

1. **[Verified] Stop UID-wide AppOps mutations from silently affecting shared system UIDs.** `AppOpsManagerCompat#setMode` uses `setUidMode`, so reset, ignore, batch, rule import, and restore paths can change every package sharing a UID. Upstream issue #2030 documents framework breakage at UID 1000. The fix belongs in one central interlock, not in scattered warnings.
2. **[Likely] Close the log-viewer detach race reported in fork issue #10.** `LogViewerViewModel#sendNewLogs` promotes a weak reference to a strong reference before posting to the main thread. `LiveLogViewerFragment#onNewLogsAvailable` then touches detached activity and view fields without a view-lifecycle check.
3. **[Verified] Move file thumbnail decoding into an isolated process.** `FmIconFetcher` sends untrusted image, PDF, font, eBook, and archive content through platform or library decoders in the app process. Android isolated services have no permissions of their own, which directly addresses the risk called out by the upstream maintainer in issue #2011, especially on API 21 through 23.
4. **[Verified] Make App Details mutations re-read authoritative state.** The privileged clear-cache branch never refreshes `sizeInfo`, matching upstream #2023. Overlay toggles notify the adapter without replacing the cached `OverlayInfoHidden`, so a successful mutation can rebind the old state.
5. **[Verified] Stop accepting app-private package-change broadcasts through an exported receiver.** `PackageChangeReceiver` combines protected system actions with `BuildConfig.APPLICATION_ID` actions under `RECEIVER_EXPORTED`, allowing an unrelated app to trigger package and database refresh work.
6. **[Verified] Add user-owned signer policies before installer commit.** The app already parses signing lineages and keeps an atomic per-package signing snapshot, but it has no strict local pin that can reject an unrecognized signer before installation. InstallerX, AppVerifier, Inure, and Obtainium issue #2922 show sustained demand for this boundary.
7. **[Verified] Make dependency-CVE results describe shipped reachability.** The 2026-08-22 SARIF contains 962 result rows across 79 artifacts, mixing release runtime, transformed wrappers, host JDBC, test, and build-tool dependencies. `scripts/run_dependency_cve_gate.py` aggregates all configurations, so reviewers cannot tell a packaged risk from a CPE-name collision without manual archaeology.
8. **[Verified] Repair the privacy policy's network contract.** `PRIVACY_POLICY.rst` still says full builds can contact Pithus even though v0.6.13 removed that integration and `NetworkTransparencyLedger` no longer lists it.
9. **[Verified] Probe backup destinations before trusting their semantics.** Neo Backup and Android DataBackup reports show that SAF, CIFS, and SMB providers can pass basic selection yet fail parallel writes, rename, retention, or restore. AppManagerNG has capacity checks and transactional publish, but no destination conformance test or stored sequential-mode fallback.
10. **[Verified] Finish several contained correctness gaps.** Main-list state still relies on color and terse symbols, empty regex replacement is rejected, Swift APK plus OBB conversion has a disabled regression test, and API 30 proxy attribution is left behind a TODO.

[Verified] The public GitHub release is still v0.6.12 from 2026-08-08 while the checked tree, README, and changelog identify v0.6.18. Fork issues #6, #8, #9, and #11 are fixed in source but not in the published v0.6.12 artifact. Publishing is an operator and signing action, so it is reported here rather than presented as coding work in `ROADMAP.md`.
...
- **[Verified] Upstream #2030, shared system UID AppOps collateral damage.** `compat/AppOpsManagerCompat.java:808` calls `setUidMode`. Reachable callers include `details/AppDetailsViewModel.java`, `permission/PermUtils.java`, `backup/RestoreOp.java`, `batchops/BatchOpsManager.java`, and `rules/compontents/ExternalComponentsImporter.java`. Android documents that security and privacy AppOps may be UID-scoped.
- **[Likely] Fork #10, crash after leaving live log view on API 29 in no-root mode.** The report is reproducible on v0.6.12. Current `logcat/LogViewerViewModel.java:219` still posts a captured fragment callback, and `logcat/LiveLogViewerFragment.java:117` still accesses detached UI state without an `onDestroyView` cancellation boundary.
...
- **[Verified] Fork #6, #8, #9, and #11 are fixed in the v0.6.18 tree.** Evidence: `CHANGELOG.md`, `settings/PrivacyPreferences.java`, `main/MainActivity.java`, and `app/src/main/res/mipmap-anydpi-v26/ic_launcher.xml`. They should be closed with the first public release containing those fixes. Fork PR #3 conflicts with and is functionally superseded by the current Android 17 and privacy fixes.
...
- **UID blast radius:** `AppOpsManagerCompat#setMode` makes package-looking actions UID-wide. A central preflight must resolve all packages for the UID, state the affected operation set, and fail closed when the operation plan did not explicitly include that shared UID.
...
- **Internal broadcast boundary:** `PackageChangeReceiver.java:68-89` registers internal and system actions together as exported. Split the filters and make `BuildConfig.APPLICATION_ID` actions non-exported or signature-protected. Validate array size before dispatch.
...
- `PackageChangeReceiver` mixes trust domains in one filter and starts a new `HandlerThread` for each received event. Separate system and private signal adapters, validate payloads at the boundary, and feed one lifecycle-owned executor.
...
- Test gaps with direct user impact remain in log-viewer teardown, UID-wide AppOps preflight, overlay readback, empty regex replacement, Swift APK plus OBB conversion, and external-broadcast rejection. Each accepted roadmap row carries an observable regression boundary.
